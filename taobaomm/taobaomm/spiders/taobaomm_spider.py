...
class Taobaomm_Spider(Spider):
    ...
    #                     所爬取网页为动态网页，翻页功能的实现通过js动态加载在同一个网页中。下面的pages_parse方法思路是，预加载
    #                  所有“页面”，通过driver.page_source存储页面源文件，供后面解析图片使用。
    #                     方法使用中反复报错 “主机积极拒绝访问，无法连接”，经调试无果，暂放弃使用。
    #                     通过观察网页源文件，发现所有“页面”待抓取图片均在首页源文件的“lazyload”属性中显示，故直接采用主parse方法
    #                  对首页源文件进行抓取。
